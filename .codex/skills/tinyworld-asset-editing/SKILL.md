...
- Brush shape modes should keep the same safety contract: Single may use the existing live freehand draw path, but Line/Rectangle drags must render a holographic footprint first and commit through `applyToolToCell()` only on pointer-up. Keep the current brush mode in `window.__tinyworldBrushModes` so the mode indicator and UI stay in sync.
